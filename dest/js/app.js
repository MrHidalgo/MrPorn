...
function initCategoryPage() {
  ...
  var initTagsContainerSlide = function initTagsContainerSlide() {// if(document.querySelector('#sidebar-all-tags') && document.querySelector('.category-list-container')){
  ...
  if (allTagsCookie == '1') {
    ...
    if (sidebarAllTags.length > 0) {
      ...
      categoryListContainer === null || categoryListContainer === void 0 ? void 0 : categoryListContainer.classList.add('show_tags');
  ...
  var initStickySidebar = function initStickySidebar() {
    ...
    var bodyClasses = document.body.classList; // if(bodyClasses.contains('tax-category-tag') && filterA2z){
...
var StickySidebar = function () {
  ...
  var StickySidebar = /*#__PURE__*/function () {
    ...
    function StickySidebar(sidebar, mainContainer) {
      ...
      var mainElement = 'string' === typeof mainContainer ? document.querySelector(mainContainer) : mainContainer; // Sidebar element query if there's no one, throw error.
    ...
    _createClass(StickySidebar, [{
      ...
      value: function destroy() {
        ...
        for (var _key4 in styleReset.inner) {
          this.sidebarInner.style[_key4] = styleReset.inner[_key4];
        }
...
var TagsShrinkEffect = /*#__PURE__*/function () {
  function TagsShrinkEffect(tagsContainerId, sidebarId) {
    ...
    this.isValid = true;
    this.tagsToFade = this.tagsContainer.querySelectorAll('span'); // Configuration: Offsets from the point the sidebar becomes sticky at the top
    ...
    this._handleResize = this._onResize.bind(this); // Initialize

    this._initialize();
  }

  _createClass(TagsShrinkEffect, [{
    key: "_initialize",
    value: function _initialize() {
      if (!this.isValid) return; // Determine the point at which the sidebar becomes sticky at the top
      // For a `position: sticky; top: 0;` element, this is its natural offsetTop.

      this.activationPoint = this.sidebar.offsetTop;

      this._captureInitialHeight(); // Set initial height explicitly for CSS transition to work from 'auto'
    ...
    value: function _captureInitialHeight() {
      ...
      this.initialTagsHeight = this.tagsContainer.scrollHeight; // Restore original style or let _updateOnScroll set it based on current scroll
    ...
    value: function _updateOnScroll() {
      ...
      var scrollY = window.scrollY; // Calculate scroll position relative to when the sidebar (and thus tagsContainer) is at the top.
    ...
    value: function _onResize() {
      if (!this.isValid) return; // Recalculate activation point and initial height as layout might change

      this.activationPoint = this.sidebar.offsetTop;

      this._captureInitialHeight(); // Re-apply styles based on new dimensions and current scroll
