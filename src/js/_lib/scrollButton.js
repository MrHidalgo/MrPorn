...
const initScrollSpyButton = ({
	...
	const getPercent = () => {
		...
		const lastSectionTop =
			windowScrollTop +
			Math.floor($lastSection.getBoundingClientRect().top) -
			_topOffset;
		return Math.min(windowScrollTop / lastSectionTop, 1);
	};
	const setPercentCSSProperty = (val = "") =>
		$buttons.forEach(($btn) => $btn.style.setProperty("--percent", val));
	const toggleTopClass = (val = 0) =>
		$buttons.forEach(($btn) =>
			$btn.classList.toggle("scroll-to-top", val >= 1)
		);
	const onScroll = () => {
	...
	const onClick = (e) => {
		...
		Array.from($sections).some(($section, idx) => {
			const sectionTop =
				windowScrollTop + Math.floor($section.getBoundingClientRect().top);
			const sectionBottom =
				windowScrollTop + Math.floor($section.getBoundingClientRect().bottom);
			const isInView =
				sectionTop < windowScrollTop + windowHeight &&
				sectionBottom > windowScrollTop + _topOffset;
			if (isInView) {
				top = $sections[idx + 1]
					? windowScrollTop +
					  Math.floor($sections[idx + 1].getBoundingClientRect().top) -
					  _topOffset
					: 0;
				return true;
			}
	...
	const toggleBindScroll = (val = true) =>
		window[`${val ? "add" : "remove"}EventListener`]("scroll", onScroll);
	const toggleBindClick = (val = true) =>
		$body[`${val ? "add" : "remove"}EventListener`]("click", debounce(onClick));
	toggleBindScroll();
	toggleBindClick();
	onScroll();
	return {
